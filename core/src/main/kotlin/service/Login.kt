        ...
        when (it.status.value) {
            200 -> {
                ...
                if (parse.getString("resultCode") == "0000") {
                    val content = Json.parseObject(Crypto.TwPay.decrypt(parse.getString("content")!!)).modify(
                        "phone" to primitive { phoneOrUserId },
                        "password" to primitive { passwordMD5 }
                    )
                    return@let Json.decodeFromJsonElement(content)
