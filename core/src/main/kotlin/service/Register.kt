    ...
    suspend operator fun invoke(phone: String = getRandomPhone(), password: String = getRandomPassword()): User {
        suspend fun getResponse(i: Int): User? = client.post(TalkwebHost.REGISTER.url) {
            ...
            setBody(
                FormDataContent(
                    Parameters.build {
                        val registerInfo = Json.encodeToString(RegisterInfo(password = Crypto.getMD5(password), phone = phone, smsVerifyCode = i.toString().padStart(4, '0')))
                        listOf(
            ...
            return@let when (it.status.value) {
                200 -> (Json.parseToJsonElement(Crypto.TwPay.decrypt(it.bodyAsText()).apply { println(this) }) as? JsonObject)
                    ?.let { response ->
                        when (response.getString("resultCode")!!) {
                            ...
                            "0000" -> {
                                logger.info("注册成功")
                                val content = Crypto.TwPay.decrypt(response.getString("content")!!).parseObject().modify(
                                    "phone" to primitive { phone },
                                    "password" to primitive { password }
                                )
...
private suspend fun register(phone: String = Register.getRandomPhone(), password: String = Register.getRandomPassword()): User = Register(phone, password)
