...
object GenVerifySms {
    suspend operator fun invoke(phone: String, smsType: SmsType = SmsType.Register): Boolean = client.post(TalkwebHost.GEN_VERIFY_SMS.url) {
        ...
        return when (it.status.value) {
            200 -> (Json.parseToJsonElement(Crypto.TwPay.decrypt(it.bodyAsText())) as? JsonObject)
                ?.let { response -> response.getString("resultCode")!! == "0000"
                } == true
            else -> false
        }
    }
